...
func TestMemoryRepository_SaveCircle(t *testing.T) {
	...
	c, err := circle.New("Book Club", user.ID("user-1"))
...
func TestMemoryRepository_FindByID(t *testing.T) {
	...
	assert.Contains(t, found.Members, user.ID("user-1"))
}
...
func TestMemoryRepository_FindByUserID(t *testing.T) {
	...
	c1, err := circle.New("Book Club", user.ID("user-1"))
	require.NoError(t, err)
	err = repo.Save(context.Background(), c1)
	require.NoError(t, err)

	c2, err := circle.New("Movie Club", user.ID("user-1"))
	require.NoError(t, err)
	...
	circles1, err := repo.FindByUserID(context.Background(), "user-1")
	require.NoError(t, err)
	assert.Len(t, circles1, 2)

	// User 2 should have 1 circle
	circles2, err := repo.FindByUserID(context.Background(), "user-2")
	require.NoError(t, err)
	assert.Len(t, circles2, 1)
	assert.Equal(t, c1.ID, circles2[0].ID)

	// User 3 should have 0 circles
	circles3, err := repo.FindByUserID(context.Background(), "user-3")
	require.NoError(t, err)
...
func TestMemoryRepository_AddMember(t *testing.T) {
	...
	// Create and save circle
	c, err := circle.New("Test Circle", user.ID("user-1"))
	require.NoError(t, err)
	err = repo.Save(context.Background(), c)
	require.NoError(t, err)
	...
	assert.Contains(t, found.Members, user.ID("user-2"))
	assert.Equal(t, 100, found.Members[user.ID("user-2")].Clout)
}
...
func TestMemoryRepository_Concurrency(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func(idx int) {
			c, _ := circle.New("Circle", user.ID("user-idx"))
