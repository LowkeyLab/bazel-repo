...
- Stable-ID card play/combat/end-turn/concede actions, ruleset-precedence natural/extra-turn scheduling, turn-series-aware stat, keyword, and ordered cost enchantments, in-Play Hero Power and Hero replacement invariants, deterministic legal actions, canonical snapshots/traces, versioned JSON checkpoints, suspended-choice restoration, and checkpoint-exact forks

All card-type action sequences, choice-producing card mechanics, temporary trigger payloads, and remaining esoteric compatibility policies are tracked in the progress document. The resolver and checkpoint API preserve and restore generic pending choices, normalized turn grants, native state, and temporary durations.
