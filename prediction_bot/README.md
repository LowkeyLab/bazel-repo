...
`/market list` offers a dropdown of the ten newest open markets. Selecting a market, or using `/market show id`, displays its question, closing time, status, pooled points, and each outcome’s implied chance. Percentages are weighted by points staked (outcome stake divided by total market stake), rounded to one decimal place. Empty markets show `N/A (no bets)`; unbacked outcomes in funded markets show `0.0%`. These percentages describe the betting pool, not a guaranteed likelihood, and rounded percentages may not total exactly 100%. Market cards are read-only. Use `/market bet` to place a bet, either through its private picker or with the direct arguments described below. Old betting controls on previously displayed cards and already-open legacy stake forms are no longer supported; they direct you to `/market bet` without staking points. Use `/market list` again to refresh pooled points and available markets.

Run `/market resolve` without arguments to open a private picker of closed, unresolved markets you can settle. Creators see their own markets; members with Administrator or Manage Guild permission see all eligible markets. Markets appear in closing-time order, oldest first, with Previous and Next buttons when there are more than 25. Select a market, choose its winning outcome, then press **Confirm resolution** to settle it. **Back** lets you change the outcome or return to the market picker without settling. The widget updates in place and checks current permissions and market eligibility again before settlement. If another member has already resolved or cancelled the market, it reports that the market is no longer eligible. Run `/market resolve` again to refresh the picker.

Run `/market bet` without arguments to open a private picker of open markets, ordered by closing time (soonest first). Use **Previous** and **Next** when there are more than 25 markets. Choose a market to see its outcomes and stake-weighted percentages, then select an outcome and enter a positive whole-number stake in the form. Enroll with `/market join` before betting. The form shows your available balance. Submitting it opens a private confirmation showing the market, outcome, and stake; points move only when you press **Confirm bet**. **Back** returns to outcome selection and **Cancel** closes the widget. Confirmation checks the current market status and available balance again. The private receipt records the outcome, stake, and remaining balance. Forms and controls belong to the member and server that opened them. Repeated confirmation clicks recover the same receipt; submitting another stake form creates a separate bet.

The direct shortcuts remain available: `/market create question options closes_at` requires all three fields, outcomes separated by `|` (for example `Yes | No`), and an RFC 3339 close time. `/market bet id outcome amount` uses the displayed **one-based** outcome number and a positive integer stake, and places the bet immediately without a confirmation widget. `/market balance` and `/market leaderboard` show current points. `/market resolve id outcome` requires both arguments and settles a closed market directly, without a confirmation widget; `/market cancel id` can be used before or after close while the market is unresolved. Resolution is available to the market creator or members with Administrator or Manage Guild permission. Cancellation requires Administrator or Manage Guild permission. Slash-command responses are private to the invoking member and do not ping users or roles.

### Market announcements

A server Administrator or member with Manage Guild permission can run `/market announcements set channel` to choose one ordinary server text channel. The bot validates that the channel belongs to the server and that its effective permissions include **View Channel** and **Send Messages** before saving it. The root `/market` command remains available to every server member. Configuration responses are private and suppress user and role pings. A welcome message is queued in the selected channel on first setup, re-enabling after disable, or changing channels. Selecting the same enabled channel (including resuming paused delivery) does not queue another welcome, and replaying a command does not duplicate it. The welcome uses the same durable delivery and retry behavior as other announcements; pending market messages retain their order. Changing channels replaces any pending welcome for the previous destination, including a welcome waiting for retry.

When enabled, the bot announces new participant registration, each accepted bet, market creation, resolution, and cancellation. Registration announcements identify the member without pinging them. Bet announcements show the market question, market ID, and total number of accepted bets (including repeat bets by the same person), plus each outcome’s stake-weighted percentage, without identifying the bettor or individual stake. Creation announcements show `N/A (no bets)` for each outcome; resolution and cancellation announcements show the final betting percentages. Bet announcements add 🟢 ⬆️ when a choice’s displayed percentage increased since immediately before that bet, and 🔴 ⬇️ when it decreased. Unchanged displayed percentages and the first bet have no arrow. Event times, bet counts, percentages, and movement indicators are captured when the action succeeds and remain unchanged during delayed delivery or retries. Older queued announcements that predate percentage snapshots remain deliverable without percentages; snapshots without movement remain deliverable without arrows. Repeated joins and redelivered commands do not enqueue extra announcements. Events that happened before announcements were enabled are not backfilled. Use `/market announcements status` to see the configured channel, whether delivery is enabled or paused, the pending count, and any safe pause reason.
