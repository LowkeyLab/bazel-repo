...
Run `/market bet` without arguments to open a private picker of open markets, ordered by closing time (soonest first). Use **Previous** and **Next** when there are more than 25 markets. Choose a market to see its outcomes and stake-weighted percentages, then select an outcome and enter a positive whole-number stake in the form. The form shows your available balance. Submitting it opens a private confirmation showing the market, outcome, and stake; points move only when you press **Confirm bet**. **Back** returns to outcome selection and **Cancel** closes the widget. Confirmation checks the current market status and available balance again. Repeated confirmation clicks recover the same receipt; submitting another stake form creates a separate bet.
