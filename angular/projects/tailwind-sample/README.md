...
When you add new Tailwind utility classes to your templates, you can use either of these approaches:

#### Automatic Update with `write_source_files` (Recommended)

```bash
bazel run //angular/projects/tailwind-sample:write-styles-css
```

This uses the `write_source_files` rule from `@aspect_bazel_lib` to:

1. Generate CSS from the source file at build time
2. Automatically update `src/styles.css` if it differs from the generated output
3. Provide a diff test to verify the file is up-to-date in CI

The diff test runs automatically during `bazel test //...`:

```bash
bazel test //angular/projects/tailwind-sample:write-styles-css_test
```

#### Manual Generation

Alternatively, use the `generate-css` target for manual generation:
...
| `tailwindcss-bin.js`      | Wrapper for manual `generate-css` target                                    |
| `tailwindcss-generate.js` | Wrapper for build-time CSS generation (used by `write_source_files`)        |
| `BUILD.bazel`             | Bazel build configuration with CSS generation and `write_source_files`      |
