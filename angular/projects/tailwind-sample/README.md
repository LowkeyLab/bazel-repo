...
You can also use the underlying generator directly if needed:
...
The primary CSS generation path uses the Bazel `write_styles_css` target, which wraps a Tailwind CLI runner to produce `styles.css`. Additionally, the `generate-css` Bazel target is a `js_binary` that:
