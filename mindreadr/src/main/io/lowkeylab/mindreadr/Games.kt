...
fun Application.configureGamesWs(gameService: GameService) {
  routing {
    webSocket("/games/{id}/live") {
      ...
      val broadcastFlow = broadcastFlows.computeIfAbsent(gameId) { MutableSharedFlow() }

      val terminationFlow = terminationFlows.computeIfAbsent(gameId) { MutableSharedFlow() }
      ...
      val broadcastJob = launch {
        broadcastFlow.asSharedFlow().collect { message ->
          ...
          if (message is OutgoingMessage.GameState) {
            if (
                message.game.state == GameState.COMPLETED ||
                    message.game.state == GameState.TERMINATED
            ) {
              log.info(
    ...
    route("/games") {
      ...
      route("/{id}") {
        get {
          ...
          val game =
              gameService.getGameById(GameId(id))
