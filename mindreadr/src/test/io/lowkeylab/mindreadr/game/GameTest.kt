...
class GameTest {
  ...
  fun `addGuess should fail when no current round exists`() {
    ...
    assertFailsWith<IllegalStateException> { game.addGuess(player1, "apple") }
  }
  ...
  fun `removePlayer should fail when game is COMPLETED`() {
    ...
    assertFailsWith<IllegalStateException> { game.removePlayer(player1) }
  }
  ...
  fun `addGuess should reject empty string`() {
    ...
    assertEquals(
        2,
        game.getRounds().first().guesses.size,
    )
  }
