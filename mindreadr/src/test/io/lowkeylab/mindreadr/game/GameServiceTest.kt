...
class GameServiceTest {
  ...
  @Test
  fun `createGame should create distinct games and be waiting for players`() = runBlocking {
  ...
  @Test
  fun `getGameById returns null for missing game and game for existing`() = runBlocking {
    val service = newService()
  ...
  @Test
  fun `addPlayerToGame transitions game to IN_PROGRESS at limit`() = runBlocking {
    val service = newService()
    val game = service.createGame()
  ...
  @Test
  fun `addGuessToGame progresses rounds and can complete game`() = runBlocking {
    val service = newService()
    val game = service.createGame()
    val p1 = service.addPlayerToGame(game.id)
    val p2 = service.addPlayerToGame(game.id)
  ...
  @Test
  fun `removePlayerFromGame during IN_PROGRESS completes game`() = runBlocking {
    val service = newService()
    val game = service.createGame()
    val p1 = service.addPlayerToGame(game.id)
  ...
  @Test
  fun `can get game counts by state`() = runBlocking {
    val service = newService()
